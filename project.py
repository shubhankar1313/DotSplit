...
def main():
    ...
    if os.path.exists(r"C:\DotSplit\\Export") == False:
        os.makedirs(r"C:\DotSplit\\Export")
    # Logo
...
def chooseOptionCreateDB(choice):
    r"For option selection of createDB()."
...
* A maximum of 10 participants can be added.\n"""
        ...
        data = pd.DataFrame(
            [["Due"] + participantList + ["Amount", "Paid By", "Transaction"]],
            index=[""],
        )
        data.to_csv(
            r"C:\DotSplit\Database\\" + DBName + ".csv", header=False, index=False
        )

        # FILE 2 - RECORD FILE

        data = pd.DataFrame([["Due", "Amount", "Paid By"]], index=[""])
        data.to_csv(
            r"C:\DotSplit\\Record\\" + DBName + "_record.csv", header=False, index=False
        )
...
def addExpense(expense, amount, paidBy, DBName):
    ...
    with open(r"C:\DotSplit\Database\\" + DBName + ".csv", "a", newline="") as csvfile1:
        ...
        csvwriter.writerow(
            [expense]
            + amountSplit(amount, paidBy, DBName)
            + [amount, paidBy]
            + [transaction(amount, paidBy, DBName)]
        )
    # File 2 - RECORD

    with open(
        r"C:\DotSplit\\Record\\" + DBName + "_record.csv", "a", newline=""
    ) as csvfile2:
        csvwriter = csv.writer(csvfile2)
        csvwriter.writerow([expense, amount, paidBy])
    invalid = 9
    selecDB(DBName)
    ...
    for member in members:
        if member != paidBy:
            transactions += (
                ...
                + paidBy
                + ":"
                + str(amountDivider(amount, len(members)))
                + "|"
            )
...
def deleteExpenseDetails(DBName):
    ...
    if expenseExists(expense, data):
        ...
        with open(
            r"C:\DotSplit\Database\\" + DBName + ".csv", "w", newline=""
        ) as csvfile1:
            csvwriter = csv.writer(csvfile1)
            csvwriter.writerows(deleteExpense(expense, data))
        # FILE 2 - RECORD FILE

        with open(r"C:\DotSplit\\Record\\" + DBName + "_record.csv") as file2:
            data2 = list(csv.reader(file2))
        with open(
            r"C:\DotSplit\\Record\\" + DBName + "_record.csv", "w", newline=""
        ) as csvfile1:
            csvwriter = csv.writer(csvfile1)
            csvwriter.writerows(deleteExpense(expense, data2))
        invalid = 12
        selecDB(DBName)
    else:
        invalid = 11
...
def settleExpense(DBName, dataUpdate, choice):
    ...
    if choice == (len(dataUpdate) + 1):
        ...
        with open(
            ...
            for entry in data[1:]:
                if (
                    dataUpdate[choice - 1].split(":")[0] in entry[-1]
                    or (
                        dataUpdate[choice - 1].split(":")[0].split("$")[1]
                        + dataUpdate[choice - 1].split(":")[0].split("$")[0]
                    )
                    in entry[-1]
                ):
                    entry[index] = 0
                    ...
                    for transaction in checkData:
                        if (
                            dataUpdate[choice - 1].split(":")[0] in transaction
                            or (
                                dataUpdate[choice - 1].split(":")[0].split("$")[1]
                                + dataUpdate[choice - 1].split(":")[0].split("$")[0]
                            )
...
def displaySettlements(DBName):
    ...
    for entry in getSettlements(DBName):
        print(
            f"{i} {entry.split('$')[0]} to {entry.split('$')[1].split(':')[0]} -> {entry.split('$')[1].split(':')[1]}"
        )
        i += 1
    if i > 1:
    ...
    with open(r"C:\DotSplit\Database\\" + DBName + ".csv") as file:
        ...
        for entry in data[1:]:
            for transaction in entry[-1].split("|"):
                if transaction.split(":")[0] in dataDict:
                    dataDict.update(
                        {
                            transaction.split(":")[0]: dataDict.get(
                                transaction.split(":")[0]
                            )
        ...
        while len(dataDict) > 0:
            for transaction in list(dataDict.keys()):
                ...
                if f"{From}${To}" in list(dataDict.keys()) and f"{To}${From}" in list(
                    dataDict.keys()
                ):
                    if dataDict.get(f"{From}${To}") > dataDict.get(f"{To}${From}"):
                        dataList.append(
                            "{}${}:{}".format(
                                ...
                                To,
                                (
                                    dataDict.get(f"{From}${To}")
                                    - dataDict.get(f"{To}${From}")
                                ),
                            )
                        )
                    ...
                    dataList.append(
                        "{}${}:{}".format(From, To, (dataDict.get(f"{From}${To}")))
                    )
                    del dataDict[f"{From}${To}"]
                    break
        return dataList
    ...
    with open(r"C:\DotSplit\Database\\" + DBName + ".csv") as file:
        ...
        if participant in data[0]:
            ...
            with open(
                ...
                csvwriter = csv.writer(csvfile1)
                csvwriter.writerows(addParticipant(participant, data, -3))
            invalid = 14
            selecDB(DBName)
    ...
    with open(r"C:\DotSplit\Database\\" + DBName + ".csv") as file:
        ...
        if participant in data[0]:
            ...
            with open(
                ...
                csvwriter = csv.writer(csvfile1)
                csvwriter.writerows(deleteParticipant(participant, data))
            invalid = 16
            selecDB(DBName)
        else:
            invalid = 10
...
def genPDF(DBName):
    ...
    with open(r"C:\DotSplit\\Record\\" + DBName + "_record.csv") as file:
        ...
        for col in data[0]:
            pdf.cell(40, 8, txt=col, ln=0, align="C", border=1)
        pdf.cell(40, 8, txt="", ln=1, align="C")
        ...
        for row in data[1:]:
            for col in row:
                pdf.cell(40, 8, txt=col, ln=0, align="C", border=1)
            pdf.cell(40, 8, txt="", ln=1, align="C")
        pdf.output(r"C:\DotSplit\\Export\\" + DBName + "_export.pdf")
...
def displayDatabases():
    ...
    for path in os.listdir(r"C:\DotSplit\Database"):
        ...
        names.append(
            [
                path[:-4],
                time.ctime(os.path.getctime(path_time)).split()[2]
                + " "
                + time.ctime(os.path.getctime(path_time)).split()[1]
                + " "
                + time.ctime(os.path.getctime(path_time)).split()[4],
            ]
        )
    return tabulate(
        names, headers=["Database Name", "Date of Creation"], stralign="center"
    )
